...
function Expense() {
  ...
  const handleSave = async () => {
    ...
    if (category === -1) {
      setError("Seleccioná una categoría de gasto.");
      return;
    }
  ...
  return (
    <View className="flex-1 bg-[#0F1113]">
      <View className="flex-row items-center justify-between px-4 py-3 border-b border-white/10">
        ...
        <Text className="text-lg font-bold text-white">Registrar Gasto</Text>
        <View className="w-10" />
      </View>
      ...
      >
        <View className="bg-[#141618] border border-white/10 rounded-3xl p-5">
          <View className="absolute inset-x-0 top-0 h-1 bg-rose-500/60 rounded-t-3xl" />
